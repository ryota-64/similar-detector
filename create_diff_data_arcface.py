...
if __name__ == '__main__':
    ...
    if opt.ML_model_type == 'diff':
                ...
                for sheet_name in sheet_names:
                    ...
                    for base_path in detected_paths:
                        for deri_path in detected_paths:
                            ...
                            try:
                                try:
                                    diff_label = derivative_paths.calc_diff(base_path.name,
                                                                            deri_path.name)
                                    # diff_label_str = DerivativePathModel.parse_diff_label_for_single_label(diff_label)
                            ...
                            if output_label_arcface(diff_label) == "":
                                continue
                            diff_label_str_list.append(output_label_arcface(diff_label))
                            if out_put_path.name.split('_')[0][0:1] == 'a':
                                labels_test[str(out_put_path.name)] = output_label_arcface(diff_label)
                            else:
                                p = random.random()
                                if p > 0.2:
                                    labels_train[str(out_put_path.name)] = output_label_arcface(diff_label)
                                else:
                                    labels_val[str(out_put_path.name)] = output_label_arcface(diff_label)
        ...
        with open(pathlib.Path(opt.data_sets_dir).joinpath(opt.dir_name).joinpath('train/train_labels.json'),
                  mode='w') as f:
            json.dump({'data': labels_train, 'label_list': list(set(diff_label_str_list))}, f, ensure_ascii=False,
                      indent=2)

        with open(pathlib.Path(opt.data_sets_dir).joinpath(opt.dir_name).joinpath('train/val_labels.json'),
                  mode='w') as f:
            json.dump({'data': labels_val, 'label_list': list(set(diff_label_str_list))}, f, ensure_ascii=False,
                      indent=2)

        with open(pathlib.Path(opt.data_sets_dir).joinpath(opt.dir_name).joinpath('test/test_labels.json'),
                  mode='w') as f:
            json.dump({'data': labels_test, 'label_list': list(set(diff_label_str_list))}, f, ensure_ascii=False,
                      indent=2)
