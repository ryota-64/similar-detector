...
class DataSet(data.Dataset):
    ...
    def __init__(self, root, labels_json_path, phase='train', input_shape=(1, 128, 128),
        ...
        self.labels_list = labels_json['label_list']
    ...
    def __getitem__(self, index):
        ...
        label_index = self.labels_list.index(self.label_dict[pathlib.Path(data_array_path).name])
        label = np.array([1 if label_index == i else 0 for i in range(len(self.labels_list))], dtype=np.int64)
        # label = np.array([value for value in self.label_dict[pathlib.Path(data_array_path).name].values()])
        if self.phase == 'train':
            return data_array.float(), label_index
        else:
            return data_array.float(), label_index, data_array_path
